...
Matrix<double> parse_kernel(string kernel)
{
    ...
    v.push_back(digit);
    if (pow(v.size(), 0.5) - int(pow(v.size(), 0.5)) > 0.0000001) {
        throw string("bad kernel format");
    }
    Matrix<double> res(pow(v.size(), 0.5), pow(v.size(), 0.5));
...
int main(int argc, char **argv)
{
    try {
        ...
        if (action == "--sobel-x") {
            ...
            Matrix<double> r(src_image.n_rows / 3 + 1, src_image.n_cols);
            Matrix<double> g(src_image.n_rows / 3 + 1, src_image.n_cols);
            Matrix<double> b(src_image.n_rows / 3 + 1, src_image.n_cols);
            slice_image(src_image, r, g, b);

            src_image = Image(src_image.n_rows / 3 + 1, src_image.n_cols);

            for (uint i = 0; i < r.n_rows; i++) {
                for (uint j = 0; j < r.n_cols; j++) {
                    src_image(i, j) = std::tuple<uint, uint, uint>(r(i, j), g(i, j), b(i, j));
                }
            }
