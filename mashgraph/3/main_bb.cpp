...
GLuint groundPointsCount; // Количество вершин у модели травинки
...
void DrawGround() {
    ...
    glDrawArraysInstanced(GL_TRIANGLES, 0, groundPointsCount, GROUND_INSTANCES / 100);                                            CHECK_GL_ERRORS
...
void RenderLayouts() {
    ...
    // Рисуем меши
    DrawSky();
    cout << "Sky Drawed" << endl;
    DrawGround();
    cout << "Ground Drawed" << endl;
    DrawGrass();
    cout << "Grass Drawed" << endl;
    glutSwapBuffers();
}
...
vector<VM::vec3> GenerateGrassPositions() {
    ...
    for (uint i = 0; i < GRASS_INSTANCES; ++i) {
        ...
        grassPositions[i] = VM::vec3(
            ...
            (i / int(sqrt(GRASS_INSTANCES))) / sqrt(GRASS_INSTANCES) + ((float)rand() / RAND_MAX / 10),
            (groundHeightMap[j * 10][k * 10] / 90.0)
        );
        k++;
...
vector<VM::vec3> GenerateGroundPositions() {
    vector<VM::vec3> groundPositions(GROUND_INSTANCES);
    uint j = 0,k = 0;
    for (uint i = 0; i < GROUND_INSTANCES; ++i) {
        // cout << groundHeightMap[sqrt(float(i)) * 10][sqrt(float(i)) * 10] << endl;
        groundPositions[i] = VM::vec3(
            (i % int(sqrt(GROUND_INSTANCES))) / sqrt(GROUND_INSTANCES),
            (i / int(sqrt(GROUND_INSTANCES))) / sqrt(GROUND_INSTANCES),
            (groundHeightMap[j][k] / 90.0)
        );
        k++;
        if (k >= sqrt(GROUND_INSTANCES)) { k = 0; j++;}
    }
...
void CreateGround() {
    ...
    vector<VM::vec3> groundPositions = GenerateGroundPositions();
    groundPointsCount = meshPoints.size();
    cout << groundPointsCount << endl;
    ...
    glGenBuffers(1, &positionBuffer);                                              CHECK_GL_ERRORS
    glBindBuffer(GL_ARRAY_BUFFER, positionBuffer);                                 CHECK_GL_ERRORS
    glBufferData(GL_ARRAY_BUFFER, sizeof(VM::vec3) * groundPositions.size(), groundPositions.data(), GL_STATIC_DRAW); CHECK_GL_ERRORS


    GLuint ind = glGetAttribLocation(groundShader, "position");                   CHECK_GL_ERRORS
    glEnableVertexAttribArray(ind);                                            CHECK_GL_ERRORS
    glVertexAttribPointer(ind, 3, GL_FLOAT, GL_FALSE, 0, 0);                   CHECK_GL_ERRORS
