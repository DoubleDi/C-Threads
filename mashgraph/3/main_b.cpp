...
void loadheightmap() {
    ...
    for (int i = 0; i < 1000; i++) {
        for (int j = 0; j < 1000; j++) {
            groundHeightMap.push_back((image->GetPixel(i, j).Red + image->GetPixel(i, j).Green + image->GetPixel(i, j).Blue) / 768.0);
            cout<< *(--groundHeightMap.end()) << endl;
        }
    }
    cout<< groundHeightMap.size() << endl;
}
...
void DrawGround() {
    ...
    glDrawArraysInstanced(GL_TRIANGLES, 0, groundPointsCount, GRASS_INSTANCES);   CHECK_GL_ERRORS
...
vector<VM::vec2> GenerateGroundPositions() {
    vector<VM::vec2> groundPositions(GRASS_INSTANCES);
    for (uint i = 0; i < GRASS_INSTANCES; ++i) {
        groundPositions[i] = VM::vec2((i % int(sqrt(GRASS_INSTANCES))) / sqrt(GRASS_INSTANCES), (i / int(sqrt(GRASS_INSTANCES))) / sqrt(GRASS_INSTANCES));
    }
...
void CreateGround() {
    ...
    groundPointsCount = meshPoints.size();
    vector<VM::vec2> groundPositions = GenerateGroundPositions();
    ...
    // Здесь мы привязываем новый буфер, так что дальше вся работа будет с ним до следующего вызова glBindBuffer
    glBindBuffer(GL_ARRAY_BUFFER, positionBuffer);                               CHECK_GL_ERRORS
    glBufferData(GL_ARRAY_BUFFER, sizeof(VM::vec2) * groundPositions.size(), groundPositions.data(), GL_STATIC_DRAW); CHECK_GL_ERRORS

    GLuint positionLocation = glGetAttribLocation(groundShader, "position");      CHECK_GL_ERRORS
    glEnableVertexAttribArray(positionLocation);                                 CHECK_GL_ERRORS
    glVertexAttribPointer(positionLocation, 2, GL_FLOAT, GL_FALSE, 0, 0);        CHECK_GL_ERRORS
    // Здесь мы указываем, что нужно брать новое значение из этого буфера для каждого инстанса (для каждой травинки)
    glVertexAttribDivisor(positionLocation, 1);                                  CHECK_GL_ERRORS
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * groundHeightMap.size(), groundHeightMap.data(), GL_STATIC_DRAW); CHECK_GL_ERRORS

    GLuint heightLocation = glGetAttribLocation(groundShader, "height");      CHECK_GL_ERRORS
